...
- [x] Integration with `oxisql` — store LOB (Large Object) data in blob storage from SQL queries: `tests/oxisql_lob_integration.rs` implements `LobRepository` wiring `MemoryBlobStore` (bytes) with `EmbeddedConnection` (SQL metadata); 7 tests: small LOB, 1 MiB LOB, delete removes both layers, missing-id returns None, 5 LOBs without collision, metadata list, SHA-256 integrity; uses `oxisql-embedded = "0.2.0"` (in-memory GlueSQL) as crates.io dev-dep; 7/7 passed (done 2026-06-19)
