...
- [x] Test fjall as storage backend for `oxisql` — wire `oxisql-embedded` to persist to fjall instead of memory: `tests/oxisql_integration.rs` added with 8 tests (basic CRUD, persistence across reconnect, multi-table isolation, 1k-row count, concurrent reads, DELETE all, UPDATE, NULL round-trip) using `oxisql-embedded = "0.2.0"` with `features = ["fjall-storage"]` as a crates.io dev-dep (no cross-workspace path coupling) (done 2026-06-19)
